...
def get_model_with_override(model_key: str) -> str:
    ...
    env_var = f"SYNOD_GROK_{model_key.upper()}"
    override = os.environ.get(env_var)
    if override:
        print(f"[Override] Using {override} (via {env_var})", file=sys.stderr)
        return override
    return MODEL_MAP.get(model_key, MODEL_MAP["fast"])
...
Features:
  ...
  - 자동 재시도 및 타임아웃 관리
    ...
    response = generate_with_retry(
        model=args.model, prompt=prompt, timeout=effective_timeout, max_retries=args.retries
    )
