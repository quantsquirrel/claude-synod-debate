...
2. The three brand emoji prefixes (🟠/🔵/🟢) on the correct model rows.
...
class TestPhase4Instructions:
    ...
    def test_branded_model_rows_use_correct_emoji(self):
        body = _phase4_text()
...
class TestBrandingConsistency:
    ...
    def test_phase4_uses_branding_module_emojis(self):
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "tools"))
        import model_branding

        body = _phase4_text()
        for key in ("claude", "gemini", "openai"):
            emoji = model_branding.BRANDING[key]["emoji"]
            assert emoji in body, f"{key} emoji {emoji!r} missing from phase4 module"
